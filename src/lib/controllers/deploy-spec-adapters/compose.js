...
const { readFileAsync, writeFileAsync, ensureDirAsync } = Promise.promisifyAll(
	require('fs-extra'),
)
...
module.exports = class GenerateDeploySpecFile {
	constructor(
		...
		this.buildComponents =
			buildComponents || _.get(attrs, 'build-components', [])
		this.configManifestPath = path.join(
			basePath,
			environmentName,
			version,
			target,
		...
		return Promise.all([cs.getConfig(), cm.getConfigManifest()])
			.tap(([config, cManifest]) => {
				if (this.mode === 'aggressive') {
					...
					return new configAutoGenerator(config, cManifest, this.mode)
						.generate()
						.then(config => {
							return cs.update(
								_.differenceWith(
				...
				return new configValidator(config, cManifest)
					...
					.then(errors => {
						if (errors.length) {
							...
							return readFileAsync(this.templatePath, 'utf8').then(template => {
								...
								_.forEach(this.buildComponents, (buildPath, name) => {
									...
									promises = promises.then(() => {
										if (!_.get(this.keyframe['components'], name)) {
											throw new Error(
												'Build component: ' + name + ' not defined in keyframe',
											)
										}
										if (
											...
											throw new Error(
												'Build component: ' +
													name +
													' repo not defined in keyframe',
											)
										}
										return ensureRepoInPath(
											_.get(this.keyframe['components'][name], 'repo', ''),
											buildPath,
										)
									})
