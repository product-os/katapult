...
const configStore = require('../config-store-adapters/all')['kubernetes']
		...
		return Promise.join(cs.getConfig(), cm.getConfigManifest())
			.tap(([config, cManifest]) => {
				if (this.mode === 'aggressive') {
					...
					return new configAutoGenerator(config, cManifest)
						.generate()
						.then(config => {
							return cs.update(
								_.differenceWith(
				...
				return new configValidator(config, cManifest)
					...
					.then(errors => {
						if (errors.length) {
							...
							return readdirAsync(this.templatePath).then(filenames => {
								let promises = []

								_.forEach(filenames, templateFileName => {
									promises.push(
										readFileAsync(
											path.join(this.templatePath, templateFileName),
											'utf8',
										).then(template => {
